...
# Change sampledate values into date objects. If you run this twice it will output "na", so you should run this in a separate code chunk. 
...
NTL.phys.data.JunethruOctober <- filter(
  NTL.phys.data, 
...
ex2 <- filter(
  NTL.phys.data, 
  year(sampledate) %in% c(1990:1999))


# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.

NTL.phys.dataninetynine <- filter(NTL.phys.data, daynum %>% c
))
```
...
#this does not show anything. Add the mean part. 
```
