...
nutrient_data <- read.csv(
  here("Data/Processed/NTL-LTER_Lake_Nutrients_PeterPaulGathered_Processed.csv"),
...
#create variables 
the parameter_ <-'tp-ug'
start_year <- 1990
end_year <- 2016 #can change year to change plot output. 

#in plot code, change aes y to y= .data[[the_parameter]]
my_plot <- nutrient_data %>% 
  filter(year4 > start_year & year4 < 2016) %>% 
  ggplot(aes(
    x = sampledate,
...
function(the_parameter, start_year, end_year)
...
selectInput(
  ...
  choices =c("tn_ug", "tp_ug", "nh34"), 
  selected="tn_ug" #if it renders it needs a value 
