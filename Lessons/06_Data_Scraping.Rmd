...
the_registrant <- webpage %>% 
  html_nodes(".table tr:nth-child(1) td:nth-child(2)") %>% #this selects everything w/ this tag
  html_text()
the_registrant
```
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant,
         Facility_name = !!the_facility_name, # the !! is saying this refers to a variable, not string.
         Facility_type = !!the_facility_type,
         Date = my(paste(Month,"-",Year))) #this seems useful but idk commands. 
...
#these ^^ two lists have to be the same size. 
...
#this will create 2018 for site 1, 2019 for site 2, and 2020 for site 3.. no thanks!
