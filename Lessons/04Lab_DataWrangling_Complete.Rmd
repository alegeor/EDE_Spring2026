...
```{r Exercise 1 & 2}
# Exercise 1: 
# filter NTL.phys.data for the year 1999 (-->1898 rows)
# what code do you need to use, based on the class of the variable?
e1 <- filter(NTL.phys.data,year4 == 1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.(-->1971 rows)
e2 <- filter(
  NTL.phys.data,
  year4 >= 1990 & year4 <= 1999 & lakename == 'Tuesday Lake'
  )

e2a <- filter(
  NTL.phys.data,
  year4 %in% 1990:1999 & str_detect(lakename, "Tuesday")
  )
...
# through 1999 only for July. (-->537 rows)
e3 <- NTL.phys.data %>% 
  filter(
