...
epa_data <- read.csv("./Data/Processed/EPAair_O3_PM25_NC1819_Processed.csv",
...
summary_PM25 <- epa_data %>% 
  drop_na(PM2.5) %>% 
  group_by(County) %>%  #County is stored as factor. 
...
```{r echo=FALSE}
#Table: Mean Particulates (2.5 mm)
knitr::kable(summary_PM25, caption = "Mean Particulates (2.5mm)", digits= c(NA, 2), 
col.names = c("County", "µg/m3")) 

#Table: Mean Ozone
knitr::kable(summary_Ozone, caption = "Mean Ozone", digits = c(NA,2), 
             col.names = c("County", "µg/m3"))
```
## Task 4: Plots
...
```{r echo=FALSE, fig.align="left", fig.height=3.5, fig.cap="Ozone levels by month, 2018"}
#Boxplot: Ozone Level Distribution:2018 

Boxplot2018Data <- epa_data %>% 
  filter(year == 2018) 
  
Boxplot2018 <-Boxplot2018Data %>% 
  ggplot(aes(x= factor(month), y=Ozone))+
  geom_boxplot(outlier.color = "#FF5349", fill="#FF5349")+
  labs(
    x="Month", 
    y="Ozone"
  )+
...
```{r echo=FALSE, fig.align="left", fig.height=3.5, fig.cap="Ozone Levels by Month, 2019"}
#Boxplot: Ozone Level Distribution:2019

Boxplot2019Data <- epa_data %>% 
  filter(year==2019)

Boxplot2019 <- Boxplot2019Data %>% 
  ggplot(aes(x=factor(month), y=Ozone))+
  geom_boxplot(outlier.color = "#FF5349", fill ="#FF5349")+
  labs(
    x="Month", 
