...
st_crs(epa_pm25_sites_sf)$epsg #this pulls only the epsg
...
#Reveal the extent of this dataset via the st_bbox() function //boundaries of dataset
st_bbox(counties_sf)
...
st_crs(huc8_sf)$epsg #reads 4326 

#View the data as a map, colored by the acreage in each
huc8map <- ggplot(data=huc8_sf)+
...
hucupperneuse <- huc8_sf %>% 
  filter(SUBBASIN == "Upper Neuse")

st_crs(hucupperneuse)

#Create a map

layerneuse <- ggplot()+
  geom_sf(data=huc8_sf, fill="gray")+
  geom_sf(data=hucupperneuse, fill="yellow", color="blue", line=0.8)
...
ggplot() + 
  ...
  scale_fill_gradient2(low="pink",high="blue",midpoint = 0.5) + 
...
enviro.atlas.raw <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses =c('HUC_12'='factor'))

atlas.pre.join <- enviro.atlas.raw %>% 
  mutate(HUC8 = substr(HUC_12, 1,8))

atlas.huc.8 <- atlas.pre.join %>% 
  group_by(HUC8) %>% 
  summarise(SumWindEnergy=sum(kWhkm2day, na.rm = TRUE))

#Join to HUC_8 features..gonna use Tidyverse

join.huc8.wind <- huc8_sf %>%
  left_join(atlas.huc.8, by=c("HUC_8" ="HUC8"))

#View the outputs

map.join.huc8 <- ggplot()+
  geom_sf(data=join.huc8.wind, aes(fill=SumWindEnergy))
